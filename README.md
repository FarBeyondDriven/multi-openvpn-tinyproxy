...
Once the `openvpn` instance is running, you can see all available environment variables that can be used in the `vpnroutes` script in the `/var/run/tinyproxy/$dev.env` file. 

By default this file will be removed when the connection is dropped.
